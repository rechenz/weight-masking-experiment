...
| **Fixed Mask**    | 79.21%        | -9.4%       | ⚠️ 固定容量限制，但比随机 mask 好得多      |
| **Subspace SVD**  | 65.35%        | -23.2%      | ❌ 数值崩塌，test loss 飙到 64，勉强爬回   |

##### 结论

1. **激活层约束几乎不影响最终性能**——Transformer 对 dropout/噪声的鲁棒性远超预期
2. **权重层约束（SVD 截断）是硬伤**——低秩截断在 ViT 上造成数值不稳定，需要重新设计（warmup、部分层施加、更慢的 rank 增长）
3. **Fixed Mask >> 随机 Mask**——固定 mask 子集让被保留的权重能稳定积累梯度，验证了「固定机能不全」优于「每次随机重新受伤」
4. 如果要模拟「机能不全→健全」，激活层约束（dropout/noise）是更安全的路径；权重层约束需要更精细的工程设计
